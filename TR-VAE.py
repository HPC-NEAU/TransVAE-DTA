...
def nfold_setting_sample(XD, XT, Y, label_row_inds, label_col_inds, measure, FLAGS, dataset, nfolds, i):
    ...
    best_param, bestperf, all_predictions, all_losses, all_auc, all_aupr = general_nfold_cv_test(XD, XT, Y,
                                                                                                 label_row_inds,
                                                                                                 label_col_inds,
                                                                                                 measure, FLAGS,
                ...
                for param3value in paramset3:
                    for lamda in lamda_set:
                        ...
                        for epochind in range(FLAGS.num_epoch):
                            ...
                            rperf, loss, rm2, auc = test(model, test_loader, FLAGS, param1value, param2value,
                                                         param3value, lamda)
                            rperf_list.append(rperf)

                            ##Set the conditions for early stopping
                            if (epochind + 1) % 5 == 0:
                                print('val: epoch:{},p1:{},p2:{},p3:{},loss:{:.5f},rperf:{:.5f}, rm2:{:.5f}'.format(
                                    epochind, param1value, param2value,
                                    param3value, loss, rperf, rm2))

                                if rperf >= max(rperf_list):
                                    torch.save(model, 'checkpoint.pth')
                                if rperf < max(rperf_list) - 0.2:
...
def general_nfold_cv_test(XD, XT, Y, label_row_inds, label_col_inds, prfmeasure, FLAGS, labeled_sets, val_sets, get_rm2,
    ...
    batchsz = 256 # FLAGS.batch_size  # 256
    ...
    for foldind in range(len(val_sets)):
        ...
        for epochind in range(FLAGS.num_epoch):
            ...
            if (epochind + 1) % 2 == 0:
                ...
                rperf_list.append(rperf)
                print(
                    'test: epoch:{},p1:{},p2:{},p3:{},loss:{:.5f},rperf:{:.5f}, rm2:{:.5f}'.format(epochind,
                                                                                                   param1value,
                                                                                                   param2value,
                                                                                                   param3value, loss,
                                                                                                   rperf, rm2))
        ...
        for drug_SMILES, target_protein, affinity in test_loader:
            pre_affinity = model(drug_SMILES, target_protein, FLAGS, param1value,
...
def experiment(FLAGS, foldcount=6):  # 5-fold cross validation + test
    ...
    for i in range(1):
        ...
        avgperf, avgloss, teststd, lossstd, avg_auc, auc_std, avg_aupr, aupr_std = nfold_setting_sample(XD, XT, Y,
                                                                                                        label_row_inds,
                                                                                                        label_col_inds,
                                                                                                        get_cindex,
                                                                                                        FLAGS, dataset,
    ...
    logging(
        "avg_perf = %.5f,  avg_mse = %.5f, std = %.5f, loss_std = %.5f,auc = %.5f, auc_std = %.5f, aupr =%.5f, aupr_std = %.5f" %
        (np.mean(perf), np.mean(mseloss), np.std(perf), np.std(mseloss), np.mean(auc), np.std(auc), np.mean(aupr),
         np.std(aupr)), FLAGS)
